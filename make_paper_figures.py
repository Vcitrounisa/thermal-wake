...
publication-ready figures into ./figs/ :
   ...
   fig_evolution.pdf/png        paper Fig. 3  build-up of the wake (4 snapshots)
   fig_steady.pdf/png           paper Fig. 4  quasi-stationary T field + |h|
   fig_zoom_centreline.pdf/png  paper Fig. 5  zoom + centreline profiles
...
# ---------------------------------------------------------------------------
#  paper Fig. 3:  build-up of the wake, |h| with streamlines
# ---------------------------------------------------------------------------
fig, axes = plt.subplots(2, 2, figsize=(7.0, 4.6), constrained_layout=True)
panel_times = [5.0, 30.0, 100.0, 400.0]
panels = [int(np.argmin(np.abs(snap_t - t))) for t in panel_times]
...
# ---------------------------------------------------------------------------
#  paper Fig. 4:  quasi-stationary state composite
# ---------------------------------------------------------------------------
...
T_plot = mask_obstacle(smooth_T(T_final))
...
# ---------------------------------------------------------------------------
#  paper Fig. 5:  zoom around the obstacle + centre-line cuts
# ---------------------------------------------------------------------------
...
T_sm     = smooth_T(T_final)
T_line   = T_sm[jc, :].copy()
hx_line  = hx_final[jc, :].copy()
T_off    = T_sm[joa, :].copy()                                   # uninterrupted cut
...
ax.plot(xc, T_line, "-", color="#1f4e9a", linewidth=2.0,
        label=r"$\mathsf{T}(x, y_c\!=\!\mathsf{R}/2)$", zorder=4)
...
hx_max = np.nanmax(hx_line)
ax2.set_ylim(-0.2 * hx_max, 1.15 * hx_max)
